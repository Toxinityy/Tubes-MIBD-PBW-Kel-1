...
export const AppAdmin = () => {
  return app.get("/input", function (req, res) {
    res.redirect("input");
  });
};
    ...
    fs.createReadStream(csvFile.tempFilePath)
      ...
      .on("end", () => {
        ...
        for (const row of results) {
          ...
          const insertSubKategoriQuery =
            "INSERT INTO SubKategori (namaSubKategori) VALUES (?)";
          connection.query(
            insertSubKategoriQuery,
            [row.namaSubKategori],
            function (err, results) {
              if (err) {
                console.log(
                  "Error inserting data into SubKategori table:",
                  err
                );
              } else {
                console.log(
                  "Data inserted into SubKategori table successfully"
                );
              }
            }
          );
          ...
          const insertTasQuery =
            "INSERT INTO Tas (warna, panjang, lebar, tinggi, foto) VALUES (?, ?, ?, ?, ?)";
          connection.query(
            insertTasQuery,
            [row.warna, row.panjang, row.lebar, row.tinggi, row.foto],
            function (err, results) {
              if (err) {
...
app.listen(8090, function() {
  console.log('Server berjalan di http://localhost:8090');
});
